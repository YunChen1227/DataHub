...
> - **sffx（本次新增）**：身份风险路由——上游 `idrisk`（应诺尔 enol `POST /enol/api/v1/doCheck`，**JSON 提交**；与 `x1`(gama)、`blk`(blacklist) 是**同一服务商、同一端点、同一信封**，故直接复用 `gamaEnvelope` 与 `signGama`：外层 `{encryptionType, appId, sign, apiKey, body}`，`sign = MD5(body 非空参数按键名 ASCII 升序拼「键值」串 + secret)` 小写 hex；`apiKey` 固定 `idRiskTagV107`；`encryptionType=2`（缺省）时 `name`/`idCard` 取 **MD5 摘要**后入 body 并参与加签，`=1` 则明文——摘要函数 `encodePII` 本次由 `blacklist.go` 的方法提到包级，两个 enol 产品共用一份实现）。响应 `{code, msg, seqNo, data{busiCode, busiMsg, result}}`，`result` 富对象 `{detail:[...]}`——每项为「风险类型码 `A`~`P`（A 涉案人员 / B 其他前科 / C 涉赌 / D 涉性 / E 交通肇事 / F 肇事肇祸 / G 妨害社会管理秩序 / H 危害公共安全 / I 金融套现 / J 金融诈骗 / K 侵犯财产 / L 制贩毒 / M 涉黑涉恐 / N 严重刑事 / O 危害国家安全 / P 在逃人员）+ 风险周期码 `0`~`6`（0 无风险，仅以 `A0` 形式出现 / 1 无可回溯日期 / 2 1年内 / 3 1-3年 / 4 3-5年 / 5 5-10年 / 6 10年及以上）」的两字符组合，如 `C2` = 涉赌人员·1年内。入参**仅** `name`+`idCard` 两项且均必填——上游 body 参数表没有 `mobile`（第三项 `tradeNo` 业务单号标**选填**，下游 x1 契约无对应字段、orchestrator 传给 `quota.Begin` 的 `tradeNo` 恒为空串，故与 gama/blacklist 一致**不注入**，`sign` 严格只覆盖实发参数），故**不能因为与 blk 同端点就沿用其三要素口径**，专属校验器 `parse.ParseIDRisk` 经 `orchestrator.WithParser` 挂载（手机号既不校验也不透传，更不许塞进上游 body——多一个参数就会把 `sign` 算错、被上游直接判签名失败）；该口径与 grsb/sfsm 相同，共用不导出的 `parseNameIDCard`，各保留独立导出入口。归一（依据 `docs/身份风险V107(1).pdf` §1.6 busiCode 表的【计费】标注，是计费正确性的唯一依据）：`busiCode=10` 查询成功【计费】→`001` 查得（`result.range` 透出 `result` 富对象 compact JSON）；`busiCode=1000` 数据未查得【计费】→`999` 查无——⚠️ **该码带【计费】，故 sffx 已登记进 `billing.billNotFoundRoutes`，查无也收费**；注意**同一服务商同一端点同一 busiCode 语义**的 `x1` 伽马，其 `1000` **不带**计费标注（不收费），口径正好相反，`blk` 则与 sffx 一致——三者必须按路由分别配置，禁止"看起来一样就复用"。其余 `1001` 余额不足 / `1002` 账户信息不存在 / `1003` appId 异常 / `1004` 产品编号异常 / `1005` 账号信息异常 / `1006` 透支余额已达上限 / `1007` 数据请求异常 / `1009` 服务尚未开通，以及全局 `code=-1` 响应异常 → 上游侧错误（不计费，`505062`）；未知 busiCode 亦按上游侧错误 fail closed，不退化成查得误计费。上游只有 `seqNo`（交易流水号）一个可对账标识，查得/查无/失败三路径均以 `UID`=`LogID`=`seqNo` 落审计，且 `seqNo` 由 `sanitizeRange` 从 `result.range` 剥掉、不透给下游。sffx 独立成域（redis db15 / datahub_sffx_db）——⚠️ **db15 是标准 Redis 的最后一个逻辑库（0~15），下一条新路由须先扩容 `databases` 或换实例**。文档 §3.2 的待签名字符串（`idCard…mobile…tradeNo…secret`）在 `upstream/idrisk_test.go` 里逐字反向验算；文档自相矛盾处已记录：§2.1 域名表给「生产 `enol.com.cn` / 测试 `testenol.cn`」，而 §3.2 Demo 的 url 是 `api.enolfax.com`（且 apiKey 写成另一产品 `loanRiskTagV23_2_1`），故地址一律由配置 `baseURL` 注入、代码不写死，联调时以上游实际下发的域名为准。**接入前需把部署机出口 IP 报给上游加白（文档 §1.2）。**
> - **dtjd（本次新增）**：多头借贷行为路由——上游 `multiloan`（守信 shouxin168 `POST /api/lightning/product/query`，**form 提交**；与 `zlf`(rental) 是**同一服务商、同一端点、同一信封**：`institution_id` 明文 + `biz_data = Base64(AES/ECB/PKCS5Padding(明文业务JSON))`，仅 `service`/`mode` 与响应主体不同）。业务 JSON 为 `{name, ident_number, phone, service, mode, licenseUrl, licenseType}` **七项全必传**（`service=financial_rent_service`、`mode=mode_loan_intent_v1`；`licenseUrl` 由启动时上传固定授权书到 OSS 后缓存复用，与 rental 共用 `uploadAuthLicense`）。入参 `name`+`idCard`+`mobile` 三要素**均必填**（`parse.ParseWithName` 前置拦截）。响应 `{resp_code, resp_msg, resp_order, timestamp, resp_data}`，`resp_data` 是约 700 个字段的富对象——决策类 `Rule_final_decision`(`Accept`/`Reject`/`Review`)、`Rule_final_weight`、`Rule_name_QJF0xx`/`Rule_weight_QJF0xx`，加因子类 `als_<时间窗 d7/d15/m1/m3/m6/m12 或 fst/lst>_<主体 id|cell>_<机构或业务类别>_<度量 allnum/orgnum/selfnum/tot_mons/avg_monnum/max_monnum/min_monnum/max_inteday/min_inteday/inteday/consnum/csinteday>`——整体序列化经 `result.range` 透出（与 blk/sffx 同口径，不做白名单，上游新增因子无需改代码）。`resp_order`（响应订单号）是唯一上游标识，成功/查无/失败三条路径都落审计（`UID`/`LogID` 同填）。**AES 密钥编码形态显式验算**（`multiLoanAESKey`：原始字节 → hex → Base64，取第一个落在 16/24/32 字节上的形态，全不合法立刻报错，绝不把非法长度交给 `aes.NewCipher`）。★ 计费：`SW0000` 认证成功【上游收费】→ `001` 计费；`SW0002` 查询无记录【不收费】→ `999`；**`SW0001` 认证失败被上游标【收费】，但因它与 `SW0002` 都只能归一到 `999` 而口径相反、`billNotFoundRoutes` 的粒度是整条路由一刀切，本路由按「下游一律查无 + 我方不向下游计费」处理**（落 `slog.Warn` + 审计上游订单号供人工对账，宁可自己吃成本也不向客户多收）——与兄弟路由 `zlf` 的 SW0001 标「不收费」**口径相反**，详见 `billing-scope` skill 第三节之二；其余 `SW0003`/`SW0017`/`SW0018`/`SW003x`/`SW004x`/`SW10xx`/`SW9999` 与「成功码但 `resp_data` 空/不可解析」均归上游侧错误、不计费。上游文档整理见 [`上游对接_多头借贷行为_守信_钉钉文档整理.md`](上游对接_多头借贷行为_守信_钉钉文档整理.md)。
> - **dtly（本次新增）**：多头履约行为路由——上游 `manyoverdue`（守信 shouxin168 `POST /api/lightning/product/query`，**form 提交**；与 `zlf`(rental)、`dtjd`(multiloan)、`snhmd`(compassblack) 是**同一服务商、同一端点、同一信封**：`institution_id` 明文 + `biz_data = Base64(AES/ECB/PKCS5Padding(明文业务JSON))`）。业务 JSON 为 `{name, ident_number, phone, service, mode, licenseUrl, licenseType}` **七项全必传**；`service` 与 dtjd/snhmd 相同（`financial_rent_service`），**`mode=mode_many_overdue_behavior` 是同端点区分产品的唯一位**——抄成别的 mode 不会报错，而是静默查到另一个产品的数据，故客户端默认值与请求契约单测都把它钉死。`licenseUrl` 由启动时上传固定授权书到 OSS 后缓存复用（与 rental/multiloan/compassblack 共用 `uploadAuthLicense`）；`institutionId`/`aesKey` 按产品分配，**不假定与 zlf/dtjd/snhmd 共用同一把密钥**，密钥形态按「原始字节 → hex → Base64」显式推导并单测覆盖，非法长度构造期即报错不静默降级。入参 `name`+`idCard`+`mobile` 三要素**均必填**（`parse.ParseWithName` 前置拦截，对齐上游 §2.5 三项全标必传）。响应 `{resp_code, resp_msg, resp_order, timestamp, resp_data}`，`resp_data` 为357 个扁平因子的富对象：`xyp_cpl00xx`（多头履约/逾期因子）、`xyp_t01`/`t02`/`t03td`/`t04`（分层履约因子）、`xyp_model_score_high`/`_mid`/`_low`（星耀Pro 小额网贷分V1 / 小额分期分V1 / 中大额分期分V1，范围 `[350,950]`，越大逾期率越低，未命中 `-1`）、`xyp_var1`~`xyp_var10`（预留）。**绝大多数因子是区间化档位输出而非原值**（通用约定 `0`=0、空=缺失、`-1`=负数；逐字段阈值见文档附录 A），网关不解释语义、不做字段白名单，整体序列化经 `result.range` 透出（与 blk/sffx/dtjd/snhmd 同口径，上游新增因子无需改代码）。`resp_order`（响应订单号）是唯一上游标识，成功/查无/失败三条路径都以 `UID`=`LogID` 落审计。归一（依据文档 §4「响应状态码对照表」19 码的「是否收费」列）：`SW0000` 认证成功【收费】→ `001` 查得计费——**含因子大多为空串、模型分为 `-1`（未命中）的结论**，那是上游给出的有效判定而非查无；`SW0002` 查询无记录【不收费】→ `999`；**`SW0001` 认证失败本产品标【不收费】→ 上游侧错误（`busiErr`）不计费**（与 `snhmd` 同，⚠️ 与 `dtjd` 的同一个 `SW0001`【收费】口径**相反**，不得互相套用），故 `999` 只来自 `SW0002`，`billNotFoundRoutes` 不登记本路由；其余 `SW0003`/`SW0017`/`SW0018`/`SW003x`/`SW004x`/`SW10xx`/`SW9999` 与文档未列的新码一律上游侧错误不计费，`SW0000` 但 `resp_data` 空/不可解析同样按错误处理（拿不到数据不得计费）。
> - **snhmd**：司南黑名单路由——上游 `compassblack`（守信 shouxin168 `POST /api/lightning/product/query`，**form 提交**；与 `zlf`(rental)、`dtjd`(multiloan) 是**同一服务商、同一端点、同一信封**：`institution_id` 明文 + `biz_data = Base64(AES/ECB/PKCS5Padding(明文业务JSON))`）。业务 JSON 为 `{name, ident_number, phone, service, mode, licenseUrl, licenseType}` **七项全必传**；`service` 与另两条相同（`financial_rent_service`），**`mode=mode_compass_black` 是同端点区分产品的唯一位**——抄成别的 mode 不会报错，而是静默查到另一个产品的数据，故客户端默认值与请求契约单测都把它钉死。`licenseUrl` 由启动时上传固定授权书到 OSS 后缓存复用（与 rental/multiloan 共用 `uploadAuthLicense`）；`institutionId`/`aesKey` 按产品分配，**不假定与 zlf/dtjd 共用同一把密钥**，密钥形态按「原始字节 → hex → Base64」显式推导并单测覆盖，非法长度构造期即报错不静默降级。入参 `name`+`idCard`+`mobile` 三要素**均必填**（`parse.ParseWithName` 前置拦截，对齐上游 §2.5 三项全标必传）。响应 `{resp_code, resp_msg, resp_order, timestamp, resp_data}`，`resp_data` 为 `black_list`（是否命中黑名单）+ `black_tag04`~`black_tag12`（短期频繁还款失败 / 短期频繁借贷 / 短期新机构频繁借贷 / 短期还款失败比例高 / 短期多机构逾期 / 中期频繁还款失败 / 短期新机构频繁还款失败 / 当前逾期较严重 / 高风险客户）共 10 个 `"0"`/`"1"` 标签的对象，整体序列化经 `result.range` 透出（与 blk/sffx/dtjd 同口径，不做字段白名单，上游将来补 `black_tag13` 无需改代码）。`resp_order`（响应订单号）是唯一上游标识，成功/查无/失败三条路径都以 `UID`=`LogID` 落审计。归一（依据文档 §4「响应状态码对照表」19 码的「是否收费」列）：`SW0000` 认证成功【收费】→ `001` 查得计费——**含标签全 `0` 的「未命中」结论**，那是上游给出的有效判定而非查无，误归 `999` 会该收的钱收不到；`SW0002` 查询无记录【不收费】→ `999`；**`SW0001` 认证失败本产品标【不收费】→ 上游侧错误（`busiErr`）不计费**，⚠️ 与兄弟产品 `dtjd` 的同一个 `SW0001`【收费】口径**相反**，两者不得互相套用——也因此本路由不存在 dtjd 那个「一条路由里两个 999 计费口径相反」的难题，`999` 只来自 `SW0002`，`billNotFoundRoutes` 不登记本路由；其余 `SW0003`/`SW0017`/`SW0018`/`SW003x`/`SW004x`/`SW10xx`/`SW9999` 与文档未列的新码一律上游侧错误不计费，`SW0000` 但 `resp_data` 空/不可解析同样按错误处理（拿不到数据不得计费）。
> - **域模型**：`x1 / v8v9 / zlf / blk / rlbd1 / rlbd2 / sfzhy / xfjy / tsfx / lxf / grgjj / grsb / sfsm / sffx / dtjd / snhmd / dtly` 各域独立（v8/v9 共用 v8v9 域与同一套 license，其余路由独立成域，见 v0.8）。跨域使用 license 一律 `505004`。
