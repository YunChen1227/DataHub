...
- **计费口径必须与上游文档逐码对齐——先读 `billing-scope` skill**。归一码只描述
  "有没有数据"，**收不收钱是另一张表**（`billing.TableFor`）。两条最容易踩的坑：
  1. 「查无」默认不计费，但**个别上游对查无也收费**（黑名单因子V35 的
     「1000 未查得【计费】」），此时要在 `billNotFoundRoutes` 登记，**不是**把
     999 改成 001。同一供应商的兄弟产品口径可能相反（伽马的 1000 就不计费），
     **逐产品看文档，禁止复用兄弟路由的结论**。
  2. 上游若下发显式计费标志字段（如 sfzhy 的 `IsCharge`），**以该字段为唯一判据**，
     不许从响应码反推。
  文档没有计费列的，**必须向上游书面确认**，不许自己假设。
...
- **每个源必须与「该源自己的文档」逐字一致（铁律，违反即返工）**：一个源一份文档
  ...
  6. **结果码全覆盖 + 逐码标注计费**：文档结果码表里每个码都要有归一化去处
     （查得/查无/上游侧错误），并**逐码抄下文档的计费标注**（见 `billing-scope`
     skill 第四节清单），处理「其余按此顺序拓展」这类开放式约定。
  7. **地址与环境**：生产/测试两套地址都写进配置注释；路径拼接方式（如
     `…/api/ws/<业务接口>`）照文档，不要自创。
- **不要动** quota / persistence / admin 后端——它们对路由完全泛化。`billing` 也
  基本泛化，唯一可能需要动的是 `billNotFoundRoutes`（仅当上游对查无也收费时登记
  一条，见 `billing-scope` skill）。
...
3b. **[internal/domain/billing/billing.go](internal/domain/billing/billing.go)** ——
   **仅当上游对「查无」也收费时**：`billNotFoundRoutes` 加 `"<route>": true`，并在
   旁边注明文档出处。绝大多数上游不需要改这里。判断依据与核对流程见 `billing-scope`
   skill；改了要同步 `billing_test.go` 与 `application/billing_scope_test.go` 的用例表。
...
17. **新建 `docs/API_接口文档与使用手册_<route>.pdf`** — **走 `api-doc` skill**，别自己
    发明格式：它固化了模板结构、版本隔离/上游隐匿铁律、逐路由计费口径与 pdf 产出命令。
    交付物**只有 PDF**（md/html 是会话临时产物，不进 docs/、不进 git）；起草时以现有
    某份 PDF 的正文为参照，改路由名、入参、`result.range` 语义与计费段。
    产出后跑 `python scripts/audit_manuals.py`，本路由那行必须全 `OK`。
