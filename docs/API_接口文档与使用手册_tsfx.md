...
> 说明：本服务对外契约与「经济能力查询服务（x1）」**完全一致**（同请求信封、同 MD5 加签、同 `head/body` 响应结构），仅**路由名**、请求参数与 `result.range` 的取值语义不同。已接入其它路由的客户改用本路由即可调用，加签逻辑无需改动。上游加密/签名/压缩等细节由本服务内部处理，**调用方无需关心**。
> 与其他版本的关键差异：① 入参为 `mobile` + `poly`（命中级别策略）；② 本服务的查询结果是一个**结构化数组**（每个手机号一条命中记录），原样序列化为 JSON 字符串置于 `body.result.range`，由调用方自行解析（见 [3.1.5 result.range 结构说明](#315-resultrange-结构说明)）；③ **只要调用成功即返回 `001` 并计费**——是否命中体现在结果记录的 `forbid` 字段，而非 `body.code`（本服务无 `999` 查无码）。
...
### 1.1 适用范围
本文档适用于接入本平台「投诉分析识别名单查询服务（tsfx）」的第三方产品技术开发人员与日常运维人员。
...
### 1.4 环境说明
- **正式环境**：使用正式账户，调用已开通接口，**按调用成功条数计费**（见 [五、计费说明](#五计费说明)）。
...
> 调用方传入的 `body` 字段一律为**明文**；上游要求的加密/签名/压缩由本服务内部完成，调用方无需做任何加密。
...
| `poly` | `C1` | String | 是 | 命中级别策略，枚举：`C1`=高危（命中门槛最低、命中类型最严重）；`C2`=敏感（命中一般、命中类型严重+次之）；`C3`=一般（命中门槛最高、全类命中） |

> 参数缺失或格式非法（手机号不符 / `poly` 非 `C1`/`C2`/`C3`）将返回 `head.errorCode = 505062`，不调用上游、不计费。
...
| 参数 | 示例 | 类型 | 说明 |
|---|---|---|---|
| `code` | `001` | String | 业务结果码。`001` = 查询成功【计费】。本服务无 `999`——是否命中见 `result.range` 的 `forbid` |
| `msg` | `成功` | String | 业务描述 |
| `reqid` | `lkf9x2...` | String | 本次请求流水号 |
| `uid` | `lgt1721198...` | String | 交易流水号（= 上游请求号，对账用） |
...
`result.range` 是命中结果**数组序列化后的 JSON 字符串**（网关原样透出上游结果，未做裁剪）。调用方应先对该字符串做一次 `JSON.parse` / 反序列化，得到一个数组，再逐条读取以下字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `callee` | String | 被查手机号（哈希/脱敏后的标识） |
| `forbid` | int | 命中状态：`0` = 未命中，`1` = 命中，`-1` = 异常（该号码无法判定） |
...
查询本账户累计成功调用的次数，用于客户侧自助监控。不返回额度上限/剩余量（本服务无额度限制）。
...
| `serviceUsed` | 累计成功调用次数（统计 `code=001`） |

> 说明：无任何额度上限拦截，仅做成功调用统计。
...
> 注：本服务**只要调用成功即返回 `001` 并计费**，是否命中投诉分析名单体现在 `result.range` 每条记录的 `forbid` 字段（`0` 未命中 / `1` 命中 / `-1` 异常），不影响计费。本服务无 `999` 查无码。
...
| `505062` | 检查 `mobile` 是否合法、`poly` 是否为 `C1`/`C2`/`C3`；若入参正常仍持续出现，记录 `logId` 联系我方 |
...
- [ ] `mobile` 以**明文**传入，`poly` 取值为 `C1`/`C2`/`C3`
