...
### 1.2 接入须知
1. 正式访问域名在接入时由我方商务提供。
2. 接入前需向我方提供贵公司**公网出口 IP**，以便加入访问白名单（详见 [六、IP 白名单](#62-ip-白名单)）。
3. 接入前需先申请开通账户，由我方分配 **`appKey`** 与 **`appSecret`**（加签密钥）。
...
| `uid` | `gama-xxxx` | String | 上游交易流水号（对账用） |
...
| `505002` | 账号信息异常 | 签名校验失败 / IP 不在白名单 |
| `505005` | 账户余额不足 | 服务额度（维度①）已用尽 |
| `505006` | 透支余额已达上限 | 成本额度达上限 |
| `505003` | 产品编号异常 | 保留 |
| `505062` | 数据请求异常 | 参数非法 / 上游我方原因 / 超时未决 / 系统错误（默认错误码） |
...
2. 提供贵公司公网出口 IP，等待加入白名单。
3. 按第二章实现加签，先在测试环境联调，再切正式环境。
4. 上线后通过额度查询接口（3.2）监控剩余量。

### 6.2 IP 白名单
- 仅白名单内的来源 IP 可访问；不在白名单将返回 `505002`。
- 若贵方使用 **VPN / 多出口 / 动态公网 IP**，请确保实际出口 IP 已报备；出口 IP 变化会导致被拒。
- 切换网络环境（如开关 VPN）后请确认出口 IP 仍在白名单内。
...
| `505002` | 检查签名算法（排序/空值剔除/UTF-8/小写 hex）、确认出口 IP 在白名单 |
| `505007` | 联系商务确认账户状态与有效期 |
| `505005 / 505006` | 额度用尽，联系商务扩容 |
