...
```json
{
  ...
  "body": {
    ...
    "result": {
      "range": "{\"whether_hit\":1,\"hit_grade\":3,\"hit_type\":[{\"scene\":\"P1\",\"m1\":0,\"m3\":1,\"m6\":2}]}"
    }
  }
}
```
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `whether_hit` | int | 是否命中：`0` = 未命中，`1` = 命中 |
| `hit_grade` | int | 命中等级，取值 `0`–`5`（数值越大风险越高） |
| `hit_type` | Array | 命中场景明细数组，每个元素对应一个风险场景（P1–P8 共 8 类） |
| `hit_type[].scene` | String | 场景标识（`P1`–`P8`） |
| `hit_type[].m1` | int | 近 1 个月在该场景的命中次数 |
| `hit_type[].m3` | int | 近 3 个月在该场景的命中次数 |
| `hit_type[].m6` | int | 近 6 个月在该场景的命中次数 |
...
> 上游侧的账户/参数/系统等异常（黑名单因子 `busiCode 1001–1009` 等）均归一为网关 `505062`，**不计费**，并经异步复查兜底。
...
| `range` | 黑名单因子结果的 JSON 字符串，需二次解析（结构见 3.1.5） |
