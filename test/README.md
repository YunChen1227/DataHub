...
- 存储按「域」划分：x1 / v8v9 / zlf / blk / rlbd1 / rlbd2 / sfzhy / xfjy / tsfx / lxf / grgjj / grsb / sfsm / sffx / dtjd / snhmd 各域独立域库。**v8 与 v9 同属 v8v9 域，共用同一套 license（appKey/secret/状态）**，但调用次数、成功查得数、操作日志按各自路由独立统计。任何域的 license（含 demo）在其它域的路由上一律鉴权失败（`505004`）。
- relay 启动会自动跑迁移（`0001`~`0004`；`0004` 会清除旧共享 demo）。demo license 由 `SEED_DEMO=1 RESET_DESTRUCTIVE=1 go run ./scripts/recreate_databases.go` 按域播种：appKey 各不相同（x1=`y89098io`、v8v9=`y890v8v9`（v9/v8 共用）、zlf=`y8909zlf`、blk=`y8909blk`、rlbd1=`y89rlbd1`、rlbd2=`y89rlbd2`、sfzhy=`y89sfzhy`、xfjy=`y890xfjy`、tsfx=`y89tsfx`、lxf=`y8909lxf`、grgjj=`y89grgjj`、grsb=`y890grsb`、sfsm=`y890sfsm`、sffx=`y890sffx`、dtjd=`y890dtjd`、snhmd=`y89snhmd`），`secret` 均为 `demo-app-secret`（harness `AppKeyFor(version)`）。
- ⚠️ **dtjd（多头借贷行为）与 snhmd（司南黑名单）都没有 mock，也没有专属查询用例**（按需求未做 mock）。两者只被通用用例覆盖：`01_health_routes`（路由可达性）、`04_found_count`（计数隔离）、`08_version_isolation`（域隔离）。这三个用例不调用上游，但**要求 `config.aliyun.e2e.yaml` 里已有 `versions.dtjd` 与 `versions.snhmd` 块**（否则路由未注册 → 401/404）。要做全场景回归，需分别补 `scripts/mock_multiloan.go`（下一个空闲端口 :9129）/ `scripts/mock_compassblack.go`（:9130）+ `test/cases/<NN>_<route>_query.go` + `test/run.ps1` 三处。协议正确性目前由 `internal/infrastructure/upstream/{multiloan,compassblack}_test.go` 的表驱动单测覆盖（请求契约 + 19 码归一 + 上游标识落审计）。
...
| `01_health_routes.go` | `/healthz` 与 x1/v9/v8/zlf/blk/rlbd1/rlbd2/sfzhy/xfjy/tsfx/lxf/grgjj/grsb/sfsm/sffx/dtjd/snhmd 各版本 query + quota 路由可达性 | healthz 返回 `ok`；各业务路由返回 JSON 信封（非 404） | relay 未起来 → 连接错误；路由未注册 → 404（dtjd/snhmd 若未在 e2e 配置里建块即会这样）；zlf/blk/rlbd1/rlbd2/sfzhy/xfjy/tsfx/lxf/grgjj/grsb/sfsm/sffx mock 未起 → 上游错误 |
...
| `04_found_count.go` | 成功查得数统计 + 无额度限制 + 路由隔离 | N 次成功 + M 次查无后 x1 `serviceUsed` 增量==N（按路由独立计数）；x1 流量不影响 v9/v8/zlf/blk/rlbd1/rlbd2/sfzhy/xfjy/tsfx/lxf/grgjj/grsb/sfsm/sffx/dtjd/snhmd 计数 | 计数漂移（并发/复查）→ 增量≠N 时 FAIL |
