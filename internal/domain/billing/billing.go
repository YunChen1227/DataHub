...
// 反例（**不要**往这里加）：dtjd 多头借贷行为的码表里 SW0001 认证失败标【收费】、
// SW0002 查询无记录标【不收费】，两者都归一到 999——本表的粒度是「整条路由的 999
// 一刀切」，加了 dtjd 就会把 SW0002 也计费，变成对下游多收钱。该路由按「下游查无 +
// 我方不计费」处理，上游侧成本靠 warn + 审计里的上游订单号人工对账
// （见 upstream/multiloan.go 与 billing-scope skill 第三节之二）。
//
// 同样**不要**加 snhmd 司南黑名单：它与 dtjd 同供应商（守信 shouxin168）、同端点、
// 同一张 SW 码表，但**本产品文档把 SW0001 认证失败标【不收费】**（dtjd 那份标收费），
// 于是客户端直接按上游侧错误返回、不归一到 999，该路由的 999 只来自 SW0002【不收费】。
// 两条路由同码不同价，这就是「逐产品看文档，禁止复用兄弟路由的结论」的活例子。
//
// 同样**不要**加 dtly 多头履约行为：与 snhmd 情形完全相同——同供应商（守信
// shouxin168）、同端点、同一张 SW 码表，**本产品文档把 SW0001 认证失败标【不收费】**，
// 客户端按上游侧错误返回、不归一到 999，该路由的 999 只来自 SW0002【不收费】。
// （依据：docs/上游对接_多头履约行为_守信_钉钉文档整理.md §4。）
