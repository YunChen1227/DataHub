...
- **对外（下游，v9/v8）**：`POST /v1/openapi/zlx/querySrmxV9` / `querySrmxV8`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；各自对接独立的经济能力上游（`docs/income_cls.md` 协议）。详见 [`docs/API_接口文档与使用手册_v9v8.pdf`](docs/API_接口文档与使用手册_v9v8.pdf)。
- **对外（下游，zlf=租赁分V2-D）**：`POST /v1/openapi/zlx/querySrmxZLF`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；`result.range` 透出上游 `score1`（500-700，[500-550]高 /(550-590]中 /(590-700]低）。详见 [`docs/API_接口文档与使用手册_zlf.pdf`](docs/API_接口文档与使用手册_zlf.pdf)。
- **对外（下游，blk=黑名单因子V35）**：`POST /v1/openapi/zlx/querySrmxBLK`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；上游富对象结果（`whether_hit`/`hit_grade`/`hit_type[P1-P8 的 m1/m3/m6]`）整体 **JSON 序列化为字符串**经 `result.range` 透出，客户自行解析。**计费口径是全仓唯一的例外：`001` 查得与 `999` 查无都计费**（上游码表对两者都标了【计费】，见 `billing.billNotFoundRoutes` 与 DESIGN §6.5）；报文形态不受影响，查无照旧回 `999`。详见 [`docs/API_接口文档与使用手册_blk.pdf`](docs/API_接口文档与使用手册_blk.pdf)。
- **对外（下游，rlbd1=人脸身份证比对一所）**：`POST /v1/openapi/zlx/querySrmxRLBD1`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；入参为 `name`+`idCard`+（`image` base64 或 `url` 二选一）；比对成功结果富对象（`score`/`msg`/`incorrect`/`sex`/`birthday`/`address`）整体 **JSON 序列化为字符串**经 `result.range` 透出——上游订单号 `order_no` **不透出**（只经审计的「上游uid/上游logId」两列供对账，见下「`result.range` 不透出上游标识」）。人脸比对无「查无」概念，故不产生 `999`。
...
- **对外（下游，sfzhy=身份证三要素核验）**：`POST /v1/openapi/zlx/querySrmxSFZHY`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；入参为 `name`+`idCard`(15/18 位)+`profilePicture`(base64 人像照片)；核验结果富对象（`Result`/`ResultMessage`/`ImageScore`）整体 **JSON 序列化为字符串**经 `result.range` 透出。核验结论本身无「查无」之分（`Result` 0–5 均是结论，同为 `001` 计费），但上游把 `IsCharge` 置 false 时归一为 `999` 不计费，故本路由**会**产生 `999`（无 `result` 节点），下游需按两分支处理。详见 [`docs/API_接口文档与使用手册_sfzhy.pdf`](docs/API_接口文档与使用手册_sfzhy.pdf)。
...
- **对外（下游，tsfx=投诉分析识别名单）**：`POST /v1/openapi/zlx/querySrmxTSFX`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；入参**仅 `mobile`**（命中级别 `poly` 已不再是下游入参）；网关对每次请求**并发查询 `C1`/`C2`/`C3` 三档**并整合成按 poly 归档的字典整体 **JSON 序列化为字符串**经 `result.range` 透出，形如 `{"C1":{"forbid":0,"hit":"未命中","callee":"..."},"C2":{...},"C3":{...}}`。本路由**只要至少一档调用成功即 `001` 计费**（对下游计一次成功查得数；对上游 kfongtech 则产生 3 次「调用成功即计费」的调用），是否命中体现在各档 `forbid`（无 `999` 查无码）；三档全失败才归上游错误、不计费。
...
- **对外（下游，grsb=背景评估）**：`POST /v1/openapi/zlx/querySrmxGRSB`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；入参**仅** `name`+`idCard`（两项均必填，**本路由不需要手机号**）；上游结果富对象（`xm`/`sfz`/`jfdw` 缴费单位 /`grsf`/`jfjs` 缴费基数 /`cbjfzt` 参保状态 /`jfsj` 缴费时间）**全字段**整体 **JSON 序列化为字符串**经 `result.range` 透出。区分「查得」（`001` 计费）与「查无」（`999` 不计费）。详见 [`docs/API_接口文档与使用手册_grsb.pdf`](docs/API_接口文档与使用手册_grsb.pdf)。
- **对外（下游，sfsm=身份证实名核验）**：`POST /v1/openapi/zlx/querySrmxSFSM`，对外契约与 x1 **完全一致**（同信封/同 MD5 加签/同 `head/body`），仅路由名不同；入参**仅** `name`+`idCard`（两项均必填，**本路由不需要手机号**）；核验结果富对象（`result` 一致性判定 /`desc` 结果描述 /`sex`/`birthday`/`address`）整体 **JSON 序列化为字符串**经 `result.range` 透出。「一致」与「不一致」都是确定结论，**同为「查得」**（`001` 计费）——是否一致看 `range` 里的 `result`/`desc`，不要用 `body.code` 判断；`999` 查无仅对应上游「无记录」。详见 [`docs/API_接口文档与使用手册_sfsm.pdf`](docs/API_接口文档与使用手册_sfsm.pdf)。
...
>
> **计费口径按路由取（v1.0）**：计入成功查得数的上游码由 `billing.TableFor(route)` 决定，**不是全仓一律「仅 001」**——上游对查无也收费的路由（当前只有 `blk`）须同步向下游计费，否则上游账单与我方统计长期对不平。计费口径与下游报文形态是两件事：`blk` 的查无照旧回 `body.code=999`，只是内部计费。逐路由口径与上游文档出处见 `billing-scope` skill，改动前先读它。

> **IP 准入（v0.7）**：网关**不再**做全局/每用户 IP 白名单校验；来源 IP 仅写入审计日志。生产环境由**阿里云 ECS 安全组**等网络层控制访问。

> **按域隔离 + demo 治理（v0.9）**：存储按「域」划分——`x1` / `v8v9` / `zlf` / `blk` / `rlbd1` / `rlbd2` / `sfzhy` / `xfjy` / `tsfx` / `lxf` / `grgjj` / `grsb` / `sfsm` / `sffx` 各域独立，各域独占一套 **PostgreSQL 库 + Redis 逻辑库 + license/appKey/secret + 记录**；**v8 与 v9 同属 `v8v9` 域，共用同一套 license**（调用次数/成功查得数/操作日志仍按路由独立统计），其余路由完全独立。跨域使用 license 一律鉴权失败（`505004` 账户信息不存在）。历史上被播种进**每个库**的同一个 demo license（`y89098io`，"一个 token 可访问所有路由"的根因）**随迁移 `0004` 自动清除**，生产启动不再播种 demo；开发态各域播种互不相同的 demo appKey。启动时另有防呆校验：两个不同的域若配置了同一个数据库或同一个 Redis 逻辑库，服务直接拒绝启动。管理后台为统一管理员登录，按路由标签页管理（v8/v9 标签展示同一份用户，统计/日志各自独立）。
...
- **对内（上游，按版本路由）**：每个版本各自对接一个上游，归一化为统一的 `UpstreamResult`（`001`查得 /`999`查无）：
  ...
  - `blk` → **黑名单因子V35 / 应诺尔**（`blacklist`，与 `gama` 同 `POST /enol/api/v1/doCheck` 端点 + 同 MD5 信封；`apiKey=blackIntV35`、`encryptionType=2`：`name/idCard/mobile` 由本服务取 **MD5** 摘要后入 body 加签；响应富对象 `result` 序列化为 JSON 字符串透出 `result.range`；`busiCode 10 查得` 与 `1000 未查得` **上游都标【计费】**，故我方对 `999` 也计成功查得数——与同端点的 `gama` 不同，切勿因"看起来一样"复用其码表）。
  - `rlbd1` → **人脸身份证比对一所 / 数脉**（`facecompare`，`POST /v4/face_id_card/yisuo/compare` **form** 提交；`sign = md5(appid&timestamp&app_security)`，明文传 `name/idcard` + `image` 或 `url`；收费类 `incorrect` 归一 `001`，不收费类/`code≠200` 归一上游侧错误；响应富对象序列化透出 `result.range`，其中上游订单号 `order_no` 被剥离不透出）。
  - `rlbd2` → **人脸身份证比对一所 / 数脉（独立凭证）**（同 `rlbd1` 的 `facecompare` 上游接口与协议，仅换一套 `appId/appSecret`，独立成域）。
  - `sfzhy` → **身份证三要素核验**（`idverify`，`POST /api/idCardThreeElements` **JSON** 提交；`signature = SHA256(升序 k=v&… + "&AppSecret=" + 商户密钥)`，明文传 `name/idCard/profilePicture`；计费以上游逐笔下发的**权威计费标志 `IsCharge`** 为准，不从 `Code`/`Result` 反推：`Code=0 且 IsCharge=true` 归一 `001` 计费（文档 §5.2 的 Result 0–5 均标计费，正常恒走此路），`Code=0 但 IsCharge=false` 归一 `999` 不计费并落 warn 供对账，`Code≠0` 归一上游侧错误；响应富对象序列化透出 `result.range`）。
  ...
  - `sfsm` → **身份证实名核验 / 数脉**（`idcheck`，`POST /v4/id_card/check` **form** 提交（文档明确"不要 json 方式"）；与 `rlbd1`/`rlbd2` 同一服务商、同一套鉴权 `sign = md5(appid&timestamp&app_security)`，`name`/`idcard` 明文传（`secretMode` 缺省不发，如需加密则 AES/ECB/PKCS5+Base64、key = `app_security`）；响应 `{msg,success,code,data{result,order_no,desc,sex,birthday,address}}`，业务字段经 `result.range` 透出、`order_no` 只落审计；`code=200` 且 `result=0` 一致 / `result=1` 不一致 **都归一 `001` 计费**（上游对两者均标"收费"，是确定结论），`result=2` 无记录归一 `999`，`code≠200`（`400`/`404`/`500`/`501`/`601`/`602`/`603`/`604`/`606`/`1001`）归一上游侧错误不计费。**入参不含手机号**）。
...
# 存储按域独立：x1/v8v9/zlf/blk/rlbd1/rlbd2/sfzhy/xfjy/tsfx/lxf/grgjj/grsb/sfsm/sffx 各一套 PG 库 + Redis 逻辑库。
# 注意 Redis 逻辑库已用到 db15（标准 Redis 仅 0~15），再加路由需先扩容 databases 或换实例。
...
versions:
  ...
  rlbd1:
    ...
    redis:    { db: 8, ... }
  # 其余路由 (rlbd2/sfzhy/xfjy/tsfx/lxf/grgjj/grsb/sfsm/sffx) 见 config.example.yaml
