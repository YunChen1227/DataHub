...
const multiLoanTestKey = "0123456789abcdef"

func multiLoanClient(t *testing.T, srv *httptest.Server) *MultiLoanClient {
	t.Helper()
	return NewMultiLoan(MultiLoanConfig{
		BaseURL:       srv.URL,
		InstitutionID: "inst-1",
		AESKey:        multiLoanTestKey,
		LicenseURL:    "https://shouwei.oss-cn-shanghai.aliyuncs.com/approve_files/x.pdf",
		LicenseType:   1,
	}, srv.Client())
}

func multiLoanReq() *model.UpstreamRequest {
...
func multiLoanServer(t *testing.T, body string) *httptest.Server {
	t.Helper()
...
// invalid key size 64——上线后一个请求都没发出去过。
func TestMultiLoanAESKeyDerivation(t *testing.T) {
	...
	// 选择的优先级 (不去猜一个合法长度的密钥是不是某种编码), 见 multiLoanAESKey 注释。
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := multiLoanAESKey(tc.key)
			if tc.wantLen == 0 {
				if err == nil {
			...
			if err != nil {
				t.Fatalf("multiLoanAESKey: %v", err)
			}
...
func TestMultiLoanIllegalKeyFailsEveryQuery(t *testing.T) {
	srv := multiLoanServer(t, `{"resp_code":"SW0000"}`)
	c := NewMultiLoan(MultiLoanConfig{BaseURL: srv.URL, InstitutionID: "i", AESKey: "badkey"}, srv.Client())
	if _, err := c.Query(context.Background(), multiLoanReq()); err == nil {
...
// 密文, 解开后七个字段名与默认 service/mode 一字不差。字段名写错 (如把 ident_number
// 写成 idCard) 线上表现是每个请求都 SW0017/SW0018。
func TestMultiLoanRequestContract(t *testing.T) {
	var (
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"resp_code":"SW0000","resp_msg":"查询成功","resp_order":"lgt1","timestamp":1790140310296,"resp_data":{"Rule_final_decision":"Review"}}`))
	}))
	defer srv.Close()

	if _, err := multiLoanClient(t, srv).Query(context.Background(), multiLoanReq()); err != nil {
	...
	plain, err := aesECBDecryptBase64(gotForm["biz_data"], []byte(multiLoanTestKey))
	if err != nil {
...
func TestMultiLoanNormalization(t *testing.T) {
	const foundData = `{"Rule_final_decision":"Review","Rule_final_weight":"10","als_m12_id_nbank_allnum":"3"}`
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			srv := multiLoanServer(t, tc.body)
			res, err := multiLoanClient(t, srv).Query(context.Background(), multiLoanReq())
...
func TestMultiLoanCarriesUpstreamOrderOnEveryPath(t *testing.T) {
	paths := []struct {
		name string
		body string
	}{
		{"查得 001", `{"resp_code":"SW0000","resp_msg":"查询成功","resp_order":"lgt-ok","resp_data":{"Rule_final_decision":"Accept"}}`},
		{"查无 999 (SW0002)", `{"resp_code":"SW0002","resp_msg":"查询无记录","resp_order":"lgt-nf"}`},
		{"查无 999 (SW0001 上游标收费)", `{"resp_code":"SW0001","resp_msg":"认证失败","resp_order":"lgt-af"}`},
		{"业务失败", `{"resp_code":"SW0042","resp_msg":"账户余额不足","resp_order":"lgt-err"}`},
	}
	for _, p := range paths {
		t.Run(p.name, func(t *testing.T) {
			srv := multiLoanServer(t, p.body)
			res, err := multiLoanClient(t, srv).Query(context.Background(), multiLoanReq())
			if err != nil {
				var be *model.UpstreamError
				if !errors.As(err, &be) {
...
// SW0001 对下游不泄露上游归因: body 侧只说「查无结果」, 归因只进服务端 warn 日志。
func TestMultiLoanAuthFailHidesUpstreamAttribution(t *testing.T) {
	srv := multiLoanServer(t, `{"resp_code":"SW0001","resp_msg":"认证失败","resp_order":"lgt-af"}`)
	res, err := multiLoanClient(t, srv).Query(context.Background(), multiLoanReq())
	if err != nil {
		t.Fatalf("Query: %v", err)
	}
	if res.Msg != "查无结果" {
		t.Fatalf("Msg = %q, 期望「查无结果」(不透出上游「认证失败」)", res.Msg)
	}
	if res.Range != "" {
		t.Fatalf("SW0001 无数据, range 应为空, 实际 %q", res.Range)
	}
}

// resp_data 富对象整体透出: 约 700 个 als_* 因子与 Rule_* 决策字段一个都不许丢、字段名
// 不许改写; 上游订单号 resp_order 只进审计, 不得出现在下游 result.range 里。
func TestMultiLoanRangeCarriesWholeRespData(t *testing.T) {
	const data = `{"Rule_final_decision":"Review","Rule_final_weight":"10","Rule_name_QJF045":"在非银机构多次申请-低度","Rule_weight_QJF045":"10","als_m12_id_nbank_finlea_orgnum":"1","als_lst_cell_nbank_csinteday":"1","als_fst_id_nbank_inteday":"267"}`
	srv := multiLoanServer(t, `{"resp_code":"SW0000","resp_msg":"查询成功","resp_order":"lgt-range","resp_data":`+data+`}`)
	...
	for _, k := range []string{
		"Rule_final_decision", "Rule_final_weight", "Rule_name_QJF045", "Rule_weight_QJF045",
		"als_m12_id_nbank_finlea_orgnum", "als_lst_cell_nbank_csinteday", "als_fst_id_nbank_inteday",
	} {
...
func TestMultiLoanAcceptsRespDataAsJSONString(t *testing.T) {
	srv := multiLoanServer(t, `{"resp_code":"SW0000","resp_msg":"查询成功","resp_order":"lgt-s","resp_data":"{\"Rule_final_decision\":\"Accept\",\"als_m1_id_nbank_allnum\":\"1\"}"}`)

	res, err := multiLoanClient(t, srv).Query(context.Background(), multiLoanReq())
	if err != nil {
		t.Fatalf("Query: %v", err)
	}
	for _, k := range []string{"Rule_final_decision", "als_m1_id_nbank_allnum"} {
		if !strings.Contains(res.Range, `"`+k+`"`) {
...
func TestMultiLoanRequeryNotReachable(t *testing.T) {
	srv := multiLoanServer(t, `{}`)
	got, err := multiLoanClient(t, srv).Requery(context.Background(), "r1")
	if err != nil {
