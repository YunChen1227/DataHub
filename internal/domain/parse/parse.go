...
// ParseComplaint 校验 tsfx (投诉分析识别名单) 入参。本路由对**每次**请求都并发查询
// C1/C2/C3 三个命中级别并整合结果返回 (见 upstream/complaint.go)，故 poly **不再作为
// 下游入参**——下游只需传 mobile。为兼容仍携带 poly 的旧客户端，body 里带了 poly 也
// 一律忽略、不校验、不参与查询 (poly 各档由网关内部固定枚举)。method/version 为固定
// 常量由上游客户端填充，同样非下游入参。
// 校验规则：mobile 必填且须为 11 位手机号。失败返回 busiCode 1007 数据请求异常
// （我方前置拦截，不调上游/不计费）。
