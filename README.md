...
>
> **计费口径按路由取（v1.0）**：计入成功查得数的上游码由 `billing.TableFor(route)` 决定，**不是全仓一律「仅 001」**——上游对查无也收费的路由（当前为 `blk`、`sffx`）须同步向下游计费，否则上游账单与我方统计长期对不平。计费口径与下游报文形态是两件事：`blk` 的查无照旧回 `body.code=999`，只是内部计费。反向的例外也存在：`dtjd` 的「认证失败」码被上游标为收费，但我方刻意**不**向下游计费（宁可自己吃成本，不可向客户多收）。逐路由口径与上游文档出处见 `billing-scope` skill，改动前先读它。
