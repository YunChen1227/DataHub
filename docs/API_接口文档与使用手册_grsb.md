# 背景评估查询服务（grsb）· API 接口文档与使用手册

> 面向接入方（客户）技术与运维人员的对外接口说明。
> 版本：grsb（背景评估）｜ 通信：HTTP + JSON ｜ 编码：UTF-8

> 说明：本服务对外契约与「经济能力查询服务（x1）」**完全一致**（同请求信封、同 MD5 加签、同 `head/body` 响应结构），仅**路由名**、**入参字段集合**与 `result.range` 的取值语义不同。已接入 x1 的客户改为本路由即可调用，加签逻辑无需改动。上游加密/账户标识等细节由本服务内部处理，**调用方无需关心**。
> **与其他版本最重要的差异：本路由的入参只有 `name` 与 `idCard` 两项且均必填，不需要手机号。** 传入 `mobile` 不会报错，但它不参与查询（会参与加签，见 2.3）。
> 查询结果是一个**结构化对象**（参保状态、缴费基数、缴费单位等），本服务将其**原样序列化为 JSON 字符串**置于 `body.result.range`，由调用方自行解析（见 [3.1.5 result.range 结构说明](#315-resultrange-结构说明)）。
...
### 1.1 适用范围
本文档适用于接入本平台「背景评估查询服务（grsb）」的第三方产品技术开发人员与日常运维人员。
...
| 参数 | 示例 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| `appKey` | `y890grsb` | String | 是 | 我方分配给客户的公开标识 |
...
> 调用方传入的 `body` 字段一律为**明文**（姓名/身份证号）。上游要求的加密由本服务内部完成，调用方无需做任何加密。
...
**示例**：`body = { "idCard": "330129199109094312", "name": "张三" }`，`appSecret = "<你的密钥>"`。

按 ASCII 升序排序后 key 顺序为 `idCard` → `name`，待签名串为：

```
idCard330129199109094312name张三<你的密钥>
```

`sign = MD5(待签名串)` 的小写十六进制值。

> 提示：拼接顺序由 key 的 ASCII 决定，请勿写死字段顺序。**加签的范围是你实际发送的 `body`**——若你额外传了本路由不需要的字段（如 `mobile`），它同样要参与加签，否则会被判 `505002`。加签代码示例与 x1 完全一致，见《API_接口文档与使用手册.md》第 2.4 节。
...
| 项目 | 内容 |
|---|---|
| 路径 | `POST /v1/openapi/zlx/querySrmxGRSB` |
| 完整地址 | `http://aiszcloud.cn:8080/v1/openapi/zlx/querySrmxGRSB`（或 `http://aiszcloud.com.cn:8080/v1/openapi/zlx/querySrmxGRSB`） |
...
> **本接口不需要手机号。** 只有上表两个字段参与查询；额外传入的字段会被忽略（但仍需参与加签）。
> `name` 缺失、`idCard` 缺失或格式非法将返回 `head.errorCode = 505062`，不调用上游、不计费。
...
| `result.range` | `"{\"cbjfzt\":\"1\",...}"` | String | **背景评估结果的 JSON 字符串**，调用方需 `JSON.parse` 后使用，结构见 3.1.5 |
...
```json
{
  ...
  "body": {
    ...
    "result": {
      "range": "{\"jfsj\":\"202603\",\"cbjfzt\":\"1\",\"jfjs\":\"6800\",\"xm\":\"石磊\",\"sfz\":\"430481199\",\"jfdw\":\"珠海科技有限公司\",\"grsf\":\"24\"}"
    }
  }
}
```
...
`result.range` 是背景评估结果对象**序列化后的 JSON 字符串**（网关原样透出上游结果，未做裁剪）。调用方应先对该字符串做一次 `JSON.parse` / 反序列化，再读取以下字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `xm` | String | 姓名 |
| `sfz` | String | 身份证 |
| `jfdw` | String | 缴费单位（如「某公司」） |
| `grsf` | String | 个人身份（上游标注为可忽略） |
| `jfjs` | String | 缴费基数 |
| `cbjfzt` | String | 参保状态：`1` = 正常参保；`2` = 暂停参保；`3` = 停止参保 |
| `jfsj` | String | 缴费时间，`yyyymm` 格式，例：`202401` |
...
> if (range.cbjfzt === "1") {
>   console.log("正常参保，缴费基数:", range.jfjs, "缴费时间:", range.jfsj);
> }
> ```
...
| 项目 | 内容 |
|---|---|
| 路径 | `GET /v1/openapi/zlx/quotaGRSB` |
| 完整地址 | `http://aiszcloud.cn:8080/v1/openapi/zlx/quotaGRSB`（或 `http://aiszcloud.com.cn:8080/v1/openapi/zlx/quotaGRSB`） |
...
> 上游侧的账户/参数/系统等异常（背景评估 `2-500`/`2-501`/`2-502`/`2-503`/`2-504`/`2-505`/`2-506`/`2-507`/`2-508`/`2-509`/`3-506`/`3-507`/`3-510` 等）均归一为网关 `505062`，**不计费**，并经异步复查兜底。

### 4.2 业务结果码 `body.code`（仅 `errorCode = 0` 时）

| code | 含义 | 上游对应 | 是否计费 |
|---|---|---|---|
| `001` | 查得数据 | `200` 请求成功 | **计费** |
| `999` | 查无结果 | `2-404` / `3-404` 没有查询到 | 不计费 |
...
| `505002` | 检查签名算法（排序/空值剔除/UTF-8/小写 hex）；确认**实际发送的每一个 body 字段**都参与了加签 |
| `505007` | 联系商务确认账户状态与有效期 |
| `505062` | 检查 `name` 与 `idCard` 是否齐全合法（本接口这两项必填、不需要手机号）；若入参正常仍持续出现，记录 `logId` 联系我方 |
...
- [ ] `name` 与 `idCard` 均以**明文**传入且都不为空（加密由网关内部处理）
- [ ] 清楚本接口**不需要手机号**；若仍传 `mobile`，已把它计入加签范围
...
| `range` | 背景评估结果的 JSON 字符串，需二次解析（结构见 3.1.5） |
