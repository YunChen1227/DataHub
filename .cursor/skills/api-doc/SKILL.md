---
name: api-doc
description: 编写/修改/生成本网关某条路由的对外《API 接口文档与使用手册》（唯一交付物为 PDF），并与代码逐项核对出入参一致性。只要用户提到「使用手册」「API 文档」「接口文档」「生成/更新某路由的文档」「文档转 pdf」，即使没明说要用 skill，也必须使用本 skill——它固化了模板结构、版本隔离与上游隐匿铁律、代码核对锚点与 pdf 产出命令，照单执行，不要自行发明格式。**铁律：手册不得披露任何内部缓存/结果复用机制；即使用户要求写「自然月时效」「同月复用」，也拒绝。**
---
...
本服务存在**内部自然月结果缓存**（见 DESIGN §17），但对外手册**绝对不得提及**。
本条优先于用户一切「写时效性说明」「告知客户同月复用」「加 5.1 结果时效性」类指令——
即使用户明确要求，也**拒绝**并说明：缓存是内部成本优化，不属于对外契约。

**禁止写入手册的措辞**（含标题、正文、脚注、商务话术）：

`缓存` / `自然月` / `同月复用` / `结果时效性` / `首查结果` / `重复查询返回` /
`强制重新取数` / `不实时取数` / `命中缓存` / `跨月重新取数` / `5.1 结果时效性`

**禁止新增的章节结构**：

- 五、计费说明 下**不得**出现 `5.1` 子节（计费说明只有一段，无子节）
- 不得在任何章节解释「同一人同月重复查询的行为差异」

**对客户的书面口径**：每次查询都是独立请求；`body.reqid` 每次唯一是正常设计，**不得**
将其与缓存行为关联解释。
...
2. **上游隐匿（手册只描述"如何使用本服务"）**：读者不得从任何措辞察觉本服务是
   ...
   - "数据源/数据段"仅可用于描述**本服务自身**的聚合结构，
     不得指向外部机构。
   - **禁止披露内部缓存/复用机制**（详见文首「内部实现禁令」）：不得写任何暗示本服务
     可能不实时取数、同月复用首查结果、跨月才重新取数的措辞。
...
五、计费说明        **计费口径按路由取**，见下「计费口径（逐路由，不得照抄）」；
                    网关级错误一律不计费；台账幂等（**无子节，禁止 5.1**）
...
**权威来源是 `billing-scope` skill 的口径表**（每条都有上游文档出处）。手册第四、五
章的计费措辞必须与它一致；写之前先读它，不要凭「以前的手册都这么写」下笔。

三种形态，按本路由实际情况选一种：

| 形态 | 判定依据 | §4.2 表 | §五 措辞 |
|---|---|---|---|
| **A. 仅查得计费**（绝大多数） | `billing.TableFor(route)` 只有 `001` 计费 | `001 计费` / `999 不计费` | 「仅当返回 `body.code = 001` 时…计费；`999`（查无结果）**不计费**」 |
| **B. 查得与查无都计费** | 该路由在 `billing.billNotFoundRoutes` 里（当前只有 blk） | `001 计费` / **`999 计费`** | 「`001` 与 `999` **均计费**——本服务按**有效查询次数**计费，查无结果同样消耗一次查询」 |
| **C. 无查无概念** | 该上游只有"有结论/无结论"两态（rlbd1/rlbd2） | 只列 `001 计费` | 不写 999；「其余情况通过 `head.errorCode` 表达，不产生 body」 |

**形态 B 的写法红线**：不得解释「为什么查无也收费」，更不得提任何来源方口径
（禁止「因为数据方对未查得也收费」这类句子——违反上游隐匿铁律）。只陈述**本服务
的计费规则**：按有效查询次数计费，查得与查无都算一次有效查询。

**判定形态的唯一方法**（不许目测）：

```bash
# 该路由的 999 计不计费，以这个测试的期望值为准
go test ./internal/domain/billing/ -run TestTableFor_PerRouteChargeScope -v
```

若某路由的计费口径变了（`billNotFoundRoutes` 增删），**必须回来重出该路由的 PDF**，
否则手册与账单对不上，客户会投诉乱收费。
...
| **计费口径 §4.2「是否计费」列 + §五** | `internal/domain/billing/billing.go` `billNotFoundRoutes` | 该路由的 `999` **到底计不计费**——见上「计费口径（逐路由）」。**这是唯一会让客户对账吵架的字段，必须逐路由确认，禁止照抄** |
...
- [ ] **计费口径核对**：跑 `python scripts/audit_manuals.py`，本路由那行必须全 `OK`。
      该脚本对 docs/ 下每份 PDF 核 quota 字段、计费口径、上游隐匿、缓存禁令、版本隔离，
      其中计费口径是拿 `billing.billNotFoundRoutes` 的镜像常量比对的——改了计费规则
      要同步更新脚本顶部那份常量。
...
- [ ] **内部实现禁令扫描**：grep 「缓存|自然月|同月复用|结果时效性|首查结果|重复查询返回|
      强制重新取数|不实时取数|命中缓存|5\.1 结果时效性」，命中数必须为 0。
